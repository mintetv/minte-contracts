...
describe('VestStream', function () {
	...
	before(async () => {
		...
		Token = await ethers.getContractFactory('MinteToken');
	...
	let currentTime;
	beforeEach(async () => {
		token = await upgrades.deployProxy(Token, [minter.address, minter.address, ethers.utils.parseEther('1000000000')], { initializer: 'initialize' });
		...
		await token.connect(minter.signer).transfer(vestStream.address, ethers.utils.parseEther('1000000000'));
	});
